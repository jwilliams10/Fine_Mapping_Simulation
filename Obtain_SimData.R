...
write.table(bim_sim1[,2],file="FineMapping_Application/Data/extract_list_sim1.txt",row.names = FALSE,col.names = FALSE,quote = FALSE)
write.table(bim_sim2[,2],file="FineMapping_Application/Data/extract_list_sim2.txt",row.names = FALSE,col.names = FALSE,quote = FALSE)

system("/data/williamsjacr/software/plink --bfile /data/DCEG_shared/data/simulated/simulated_multi_ances_genotype_600K/EUR/all_chr.tag --extract FineMapping_Application/Data/extract_list_sim1.txt --make-bed --out FineMapping_Application/Data/Sim1")
system("/data/williamsjacr/software/plink --bfile /data/DCEG_shared/data/simulated/simulated_multi_ances_genotype_600K/EUR/all_chr.tag --extract FineMapping_Application/Data/extract_list_sim2.txt --make-bed --out FineMapping_Application/Data/Sim2")
